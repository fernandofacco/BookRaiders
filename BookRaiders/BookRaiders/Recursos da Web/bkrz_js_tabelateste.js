...
function verificarCpf(executionContext) {
    ...
    var cpf = formContext.getAttribute("bkrz_cpf").getValue();
    ...
    if (cpf != null) {
        ...
        if (cpf.length != 11) {
            ...
            formContext.getControl("bkrz_cpf").setNotification("Formato incorreto. Campo deve conter 11 numeros", notifId);
            return;
        }

        // Obtem o controle do campo bkrz_cpf e remove uma notificacao com id
        formContext.getControl("bkrz_cpf").clearNotification(notifId);

        cpf = formatarCpf(cpf);

        // Obtem o campo bkrz cpf e altera o valor contido nele
        formContext.getAttribute("bkrz_cpf").setValue(cpf);
    }
}
